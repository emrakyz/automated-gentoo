...
handle_disk() {
    ...
    IFS=$'\n' BLOCK_DEVICES=($(lsblk -lnfo NAME,SIZE |
	                       sed 's/^/\/dev\//
			            /^[^ ]*[a-mo-z][0-9] /d'))
    ...
    read SELECTED_DEVICE && CHOSEN_DEVICE=${BLOCK_DEVICES[SELECTED_DEVICE - 1]}
    CHOSEN_DEVICE=${CHOSEN_DEVICE%% *}

    echo -e "${GREEN}The chosen device is $CHOSEN_DEVICE"
    echo -en "Do you confirm? (y/n): ${WHITE}"
    read DISK_CONFIRMATION

    echo -e "${NC}"

    parted -s $CHOSEN_DEVICE print | sed -n 's/^ *\([0-9]\) .*/\1/p' | while read part; do
   ...
   BOOT_PARTITION="$(lsblk "$CHOSEN_DEVICE" -lnfo NAME | sed -n 's/^/\/dev\//; 2p')"
	...
	[[ $CHOSEN_FS != "" ]] && {
	    ...
	    echo -en "${WHITE}Do you confirm (y/n): ${CYAN}"
	    read FS_CONFIRM
	} || {
	...
	[[ $FS_CONFIRM =~ ^[yY](es)?$ ]] && break || echo -e "${RED}You have not confirmed. Restarting..."
    done

    ROOT_PARTITION="$(lsblk "$CHOSEN_DEVICE" -lnfo NAME | sed -n 's/^/\/dev\//; 3p')"

    [[ $CHOSEN_FS == "f2fs" ]] && {
        mkfs.f2fs -a 1 \ # heap-based allocation.
          -d 0 \ # minimize debugging output.
          -f \ # non-interactive creation.
          -i \ # enable extended node bitmap.
          -s 1 \ # assign one segment to each section.
          -t 1 \ # enable trim/discard.
	  -w 4096 \ # set the sector size (4096 is best for SSDs).
          -z 1 \ # one section for each zone.
          $ROOT_PARTITION # the partition name.
        } || mkfs.ext4 $ROOT_PARTITION # if not f2fs, use ext4.
...
select_profile() {
    ...
    read profile_choice && PROFILE=${AVAILABLE_PROFILES[profile_choice - 1]}

    [[ -z $PROFILE ]] && { log_info r "Invalid selection. Exiting."; exit 1; }
...
find_tarball_url() {
    ...
    URL_GENTOO_TARBALL="$(curl "$URL_GENTOO_WEBSITE" |
        sed -n 's/.*href="\([^"]*'"$PROFILE"'-[^"]*\.tar\.xz\)".*/\1/p')"
...
detect_root_partition() {
    IFS=$'\n' partitions=($(lsblk -lnfo NAME,FSTYPE,SIZE |
	                    sed -E 's/^/\/dev\//
	                            /ext4|f2fs/!d
		                    /^[^ ]*[a-z] /d
		                    /^[^ ]*n[0-9][^p] /d
    ...
    [[ $CONFIRMATION =~ [yY](es)? ]] || { log_info r "The user has not confirmed the selection."
    exit 1; }
}
...
main() {
    ...
    task_order=(handle_disk select_profile find_tarball_url detect_root_partition mount_root_partition
	        retrieve_the_tarball extract_the_tarball remove_the_tarball
		create_repos_conf create_resolv_conf prepare_chroot execute_chroot)
